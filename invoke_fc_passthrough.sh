...
#add nova instance to apparmor complain profiles

APPARMOR=$(ssh -i $PROVIDER_KEY $PROVIDER_USER@$PROVIDER "sudo aa-complain /etc/apparmor.d/libvirt/libvirt-$NOVA_ID")
apparmor_result=$?
echo "APPARMOR result: $APPARMOR"
...
for pci in $fc_pci_device; do
    ...
    echo "fc-device:$fc_virsh_device"
...
echo $(systool -c fc_host -v | grep -B12 'Online')
device_path=$(systool -c fc_host -v | grep 'Device path')
echo ${#device_path}
...
echo "Successfully attched FC host"
